...
--report sidecar assertions (TC-SPC-XBS-016 to TC-SPC-XBS-018):

  TC-SPC-XBS-016  --report → exit 0, HTML report written to ~/Downloads/
  TC-SPC-XBS-017  --report → JSON sidecar (*_data.json) written alongside HTML
  TC-SPC-XBS-018  JSON sidecar: report_type == "pv", verdict_pass is True for stable data
"""
import sys
...
class TestXbarS:
    ...
    def test_tc_spc_xbs_006_png_created(self):
        ...
        assert recent, (
            f"No *_jrc_spc_xbar_s.png found in ~/Downloads/ after run\n"
            f"  DOWNLOADS={DOWNLOADS!r} (exists={os.path.isdir(DOWNLOADS)})\n"
            f"  All matches (any age): {glob.glob(os.path.join(DOWNLOADS, '*_jrc_spc_xbar_s.png'))!r}\n"
...
class TestXbarSReport:

    def test_tc_spc_xbs_016_report_html_created(self):
        """
        TC-SPC-XBS-016:
        ...
        """
        t_start = time.time()
        r = run("jrc_spc_xbar_s.R", data("xbar_s_stable.csv"), "--report")
        assert r.returncode == 0, f"Expected exit 0:\n{combined(r)}"
        docx_files = [
            f for f in glob.glob(os.path.join(DOWNLOADS, "*_xbar_s_pv_report.docx"))
            if os.path.getmtime(f) >= t_start - 1.0
        ]
        html_files = [
            f for f in glob.glob(os.path.join(DOWNLOADS, "*_xbar_s_pv_report.html"))
            if os.path.getmtime(f) >= t_start - 1.0
        ]
        assert docx_files or html_files, (
            f"No report file (*_xbar_s_pv_report.docx or .html) found in ~/Downloads/\n"
            f"  DOWNLOADS={DOWNLOADS!r} (exists={os.path.isdir(DOWNLOADS)})\n"
            f"  All docx: {glob.glob(os.path.join(DOWNLOADS, '*_xbar_s_pv_report.docx'))!r}\n"
            f"  All html: {glob.glob(os.path.join(DOWNLOADS, '*_xbar_s_pv_report.html'))!r}\n"
            f"  Script output: {combined(r)}"
        )

    def test_tc_spc_xbs_017_report_json_sidecar_created(self):
        """
        TC-SPC-XBS-017:
        ...
        json_files = [
            f for f in glob.glob(os.path.join(DOWNLOADS, "*_xbar_s_pv_report_data.json"))
            if os.path.getmtime(f) >= t_start - 1.0
        ]
        assert docx_files or json_files, (
            f"No report data (*_xbar_s_pv_report.docx or *_data.json) found in ~/Downloads/\n"
            f"  DOWNLOADS={DOWNLOADS!r} (exists={os.path.isdir(DOWNLOADS)})\n"
            f"  All docx: {glob.glob(os.path.join(DOWNLOADS, '*_xbar_s_pv_report.docx'))!r}\n"
            f"  All json: {glob.glob(os.path.join(DOWNLOADS, '*_xbar_s_pv_report_data.json'))!r}\n"
            f"  Script output: {combined(r)}"
        )

    def test_tc_spc_xbs_018_report_json_content(self):
        """
        TC-SPC-XBS-018:
        JSON sidecar contains report_type == "pv" and verdict_pass == True
        for a stable in-control dataset (TC-SPC-XBS-002 confirms IN CONTROL verdict).
