...
--report sidecar assertions (TC-SPC-P-014 to TC-SPC-P-016):
...
class TestPChart:
    ...
    def test_tc_spc_p_006_png_created(self):
        ...
        assert recent, (
            f"No *_jrc_spc_p.png found in ~/Downloads/ after run\n"
            f"  DOWNLOADS={DOWNLOADS!r} (exists={os.path.isdir(DOWNLOADS)})\n"
            f"  All matches (any age): {glob.glob(os.path.join(DOWNLOADS, '*_jrc_spc_p.png'))!r}\n"
...
class TestPChartReport:
    ...
    def test_tc_spc_p_014_report_html_created(self):
        ...
        """
        t_start = time.time()
        r = run("jrc_spc_p.R", data("p_stable.csv"), "--report")
        assert r.returncode == 0, f"Expected exit 0:\n{combined(r)}"
        docx_files = [
            f for f in glob.glob(os.path.join(DOWNLOADS, "*_spc_p_pv_report.docx"))
            if os.path.getmtime(f) >= t_start - 1.0
        ]
        html_files = [
        ...
        assert docx_files or html_files, (
            f"No report file (*_spc_p_pv_report.docx or .html) found in ~/Downloads/\n"
            f"  DOWNLOADS={DOWNLOADS!r} (exists={os.path.isdir(DOWNLOADS)})\n"
            f"  All docx: {glob.glob(os.path.join(DOWNLOADS, '*_spc_p_pv_report.docx'))!r}\n"
            f"  All html: {glob.glob(os.path.join(DOWNLOADS, '*_spc_p_pv_report.html'))!r}\n"
            f"  Script output: {combined(r)}"
        )

    def test_tc_spc_p_015_report_json_sidecar_created(self):
        """
        TC-SPC-P-015:
        ...
        assert docx_files or json_files, (
            f"No report data (*_spc_p_pv_report.docx or *_data.json) found in ~/Downloads/\n"
            f"  DOWNLOADS={DOWNLOADS!r} (exists={os.path.isdir(DOWNLOADS)})\n"
            f"  All docx: {glob.glob(os.path.join(DOWNLOADS, '*_spc_p_pv_report.docx'))!r}\n"
            f"  All json: {glob.glob(os.path.join(DOWNLOADS, '*_spc_p_pv_report_data.json'))!r}\n"
            f"  Script output: {combined(r)}"
        )

    def test_tc_spc_p_016_report_json_content(self):
        """
        TC-SPC-P-016:
        JSON sidecar contains report_type == "pv" and verdict_pass == True
        for a stable in-control dataset (TC-SPC-P-002 confirms IN CONTROL verdict).
